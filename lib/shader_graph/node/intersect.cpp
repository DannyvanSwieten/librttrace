...
namespace shadergraph {
	...
	void Intersect::output_instruction(CompilerContext& ctx, IOContext& io_ctx)
	{
		...
		io_ctx.set_instruction(m_output_index, instructions::Intersect{ .origin = origin, .direction = direction, .acc_index = index }, ctx);
	}
	void Intersect::add_io(IOContext& io_ctx)
	{
